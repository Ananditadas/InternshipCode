...
function createVisitApp(x)
	...
	function createChatSession(x)
	{
		...
		sesID = sesRef.push({ 
            ...
            console.log("chat doc",doc_noti,"pat",pat_noti);
			
			updates['/users/' +DID+'/chat_appointments'] = docApp;
			updates['/users/' +DID+'/new_notification'] = doc_noti;
			...
			if(doctor.locations != null)
			{
				...
				{
					updates['/locations/'+doctor.locations[i]+'/'+DID+'/chat_appointments'] = docApp;
				}
			}
			updates['/professions/'+ doctor.profession+'/' +DID+'/chat_appointments'] = docApp;
				...
				var pat_msg = "Your appointment for online consultancy with "+doctor_name+" has been sent. Please check your dashboard to proceed. Thank you! https://tx.gl/r/1Kpy/#AdvdTrack#";
...
"VIP Bypass" 
        ...
        if(SearchString.length!=0)
        {
		  for(var i=0; i<doctors_list.length; i++)
		  {
			if(doctors[i][10]=="show" && doctors[i][22]=="show" && doctor_location_boolean[i]==true)
			{
				...
				output += "<tr><td width='20%' rowspan='2' align='center'><img src='"+doctors[i][8]+"' width='90vw' height='90vw' class='doc-dp'></td>";
				
				output += "<td colspan='2'><a href='profile.html?id="+doctors[i][0]+"' class='doctor-name'>"+doctors[i][1]+"</a></td></tr>";
                
                output+="<tr><td width='60%'><table >";
				if(doctors[i][5]==-1)
				{
	...
	function onlineModal(x)
	{
		...
		{	
			...
			doctor =doctors[x][16];
			console.log(doctors[x][17]);
			console.log(doctors[x][18]);
