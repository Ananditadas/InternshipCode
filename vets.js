window.onload = function(){
  ...
  var ref = firebase.database().ref('professions/vet');

  var doctors = new Array();//array containing the doctors inoformation.


  firebase.database().ref('professions/vet').on('value', function(snapshot) {
    ...
    for(var i=0;i<doctors.length;i++)
    {
      ...
      var details = "";
      details="<h3>Dr "+doctors[i].first_name+" "+doctors[i].last_name+"</h3></br></br><p class=details>"+doctors[i].degrees+"</br>Physiotherapist</br>Experience: <span id=ep>"+doctors[i].experience+"</span> years</br>"+doctors[i].services[0]+" Rs."+doctors[i].rates[0]+"/-";
    ...
    for(var i=0;i<doctors.length;i++)
    {
      var m = Number($("#doc"+i).find("#ep").text());
      console.log(m);
    }
